...
export function makeWASocket(connectionOptions, options = {}) {
  /**
   * @type {import('@shizodevs/shizoweb').WASocket | import('@shizodevs/shizoweb').WALegacySocket}
   */
          ...
          if (/^https?:\/\//i.test(buffer)) {
              try {
                  ...
                  if (/^image\//i.test(contentType)) {
                      ...
                      console.error("sorry File Type Not Supported", contentType)
                  }
              } catch (error) {
                  console.error("Sorry Error getting File type", error)
              }
          } else {
    ...
    sendPayment: {
      async value(jid, amount, currency, text = '', from, options) {
        const requestPaymentMessage = {
          amount: {
            currencyCode: currency || 'KSH',
            offset: 0,
            value: amount || 10000000,
          },
          expiryTimestamp: 0,
          amount1000: (amount || 10000000) * 1000,
          currencyCodeIso4217: currency || 'KSH',
        ...
        for (const message of m) {
          try {
            ...
            if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
              /**
               * @type {import('@shizodevs/shizoweb').proto.IContextInfo}
               */
              let context = message.message[mtype].contextInfo
              let participant = conn.decodeJid(context.participant)
              const remoteJid = conn.decodeJid(context.remoteJid || participant)
              /**
               * @type {import('@shizodevs/shizoweb').proto.IMessage}
               *
               */
